...
Class AdminParserController extends Controller {
	...
	public function index() 
	{
		...
		return view('iteach.admin.uploadCSVFile', compact('requests', 'Nrequests'));
	}
	...
	public function processCSV(){
		...
		$Nrequests = AdminController::getRequests()['Nrequests'];
			...
			for($i=1;$i<$counter;$i++){
				 ...
				 if(COUNT($results) == 0){
					 $addDb = new Section;
					 $addDb->sid = str_replace(' ', '', strtolower($course_num[$i].$section[$i]));
					 $addDb->type = $type[$i];
			...
			return view("iteach.admin.successfulUpload",compact('data','requests','Nrequests'));		//add a second parameter here to pass the data to the view.
		}
		else{
		...
		$Nrequests = AdminController::getRequests()['Nrequests'];
			$data= $trueFileName;
			return view("iteach.admin.failUpload",compact('data','requests','Nrequests'));
		}
	}
